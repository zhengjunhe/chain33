...
func testVersionOK(t *testing.T) {
	reply := &types.VersionInfo{DplatformOS: "6.0.2"}
	qapi.On("Version").Return(reply, nil)
	data, err := g.Version(getOkCtx(), nil)
	assert.Nil(t, err, "the error should be nil")
	assert.Equal(t, "6.0.2", data.DplatformOS, "reply should be ok")
}
...
func TestGrpc_GetFork(t *testing.T) {
	types.RegFork("para", func(cfg *types.DplatformOSConfig) {
	...
	cfg1 := types.NewDplatformOSConfig(types.GetDefaultCfgstring())
	Init(cfg1)
