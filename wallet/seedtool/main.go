...
func genaddrlist(seed string) (map[string]bool, error) {
	...
	var err error
	if *oldseed {
		wallet, err = bipwallet.NewWalletFromSeed(bipwallet.TypeDpom, uint32(types.SECP256K1), []byte(seed))
		if err != nil {
			log.Println("GetPrivkeyBySeed NewWalletFromSeed", "err", err)
			return nil, types.ErrNewWalletFromSeed
		}
	} else {
		wallet, err = bipwallet.NewWalletFromMnemonic(bipwallet.TypeDpom, uint32(types.SECP256K1), seed)
		if err != nil {
