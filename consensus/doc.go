...
运行方式（3个节点）:
	...
	步骤：
		//  1.编译版本
		cd $GOPATH/src/gitlab.33.cn/dplatform
		git clone git@gitlab.33.cn:dplatform/dplatform.git
		cd dplatform
		go build

		//	2.从管理机上拷贝可执行文件到各个节点
		scp dplatform ubuntu@172.31.8.229:/home/ubuntu/
		scp dplatform ubuntu@172.31.15.241:/home/ubuntu/
		scp dplatform ubuntu@172.31.4.182:/home/ubuntu/

		// 3.在各个节点上，依次修改dplatform.toml
			name="raft"
			minerstart=true
             ...
             动态加入节点
		    	1.）先在原有的集群随意一台机器（leader和follower都可以）上面执行
		    	    ...
		    	    注意：前者为要加入的nodeId 值，后面http地址为要加入的peerUrl地址，将peerURL依次追加到addPeersURL中,
   	                      用逗号分隔,一次只能添加一个节点
		    	2.）然后在dplatform.toml配置文件中，写好相关参数，启动dplatform即可,
                    dplatform.toml配置文件可依据前一个节点的配置
                    修改如下参数:
                     nodeId=x             //第几个节点
...
3. pbft:
   ...
   首先下载pbftbranch，然后创建code.aliyun.com/dplatform文件夹，将解压出来的dplatform文件夹放入下面，进入dplatform文件夹
   执行go build命令生成可执行文件
   因为我是在一台电脑上进行测试，因此只需要在本机上拷贝文件即可，若需要在不同的电脑上进行测试，拷贝到不同电脑上即可
   修改个节点的dplatform.toml即可，主要修改方式如下：
   name = "pbft"
   genesis = "14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
   minerstart = true
   ...
   //TODO 动态增加或者删除节点
  然后dplatform命令启动所有节点后，再向mempool写入交易validator
...
运行方式（4个节点）:
	...
	步骤：
		...
		//	2.从管理机上拷贝可执行文件到各个节点
		scp dplatform ubuntu@192.168.0.117:/home/ubuntu/
		scp dplatform ubuntu@192.168.0.119:/home/ubuntu/
		scp dplatform ubuntu@192.168.0.120:/home/ubuntu/
		scp dplatform ubuntu@192.168.0.121:/home/ubuntu/

		// 3.在各个节点上，依次修改dplatform.toml中[consensus]项
			name="tendermint"
			minerstart=false
			genesis="14KEKbYtKKQm4wMthSK9J4La4nAiidGozt"
			genesisBlockTime=1514533394
       ...
       // 5.动态增删节点
             动态加入节点
		    	1.）将新生成的priv_validator.json文件和其他Validator节点的genesis.json和dplatform.toml拷贝到待加入集群的节点工作目录下，然后启动dplatform。
					注意：可以修改dplatform.toml文件的[p2p]项和[consensus]项中的seeds参数，只要是集群节点就可以。
